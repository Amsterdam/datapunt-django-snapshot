...
current_version=$(bump2version --dry-run --list $1 | grep "current_version=" | grep -Eo "[0-9]+.[0-9]+.[0-9]+") || exit;
...
next_version=$(bump2version --list $1 | grep "new_version=" | grep -Eo "[0-9]+.[0-9]+.[0-9]+") || exit;
today=$(date '+%d-%m-%Y')

current_changelog=$(cat Changelog.md)
cat > Changelog.md <<EOF
## ${next_version} (${date "+%d-%m-%Y"})
### Note worthy changes
$changes
